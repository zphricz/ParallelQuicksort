...
template <typename T> void insertion(T &list, int start, int end) {
...
template <typename T> void serial_qsort(T &list, int start, int end) {
...
template <typename T> void parallel_qsort(T &list, int start, int end) {
  ...
  int run_end;
  partition(list, start, end, run_start, run_end);
  Threadpool::submit_task(parallel_qsort<T>, std::ref(list), start,
                          run_start - 1);
  Threadpool::submit_task(parallel_qsort<T>, std::ref(list), run_end + 1, end);
}

template <typename T> void qsort(T &list, int size) {
  parallel_qsort(list, 0, size - 1);
  Threadpool::wait_for_all_jobs();
}

// T must implement the size() method to work
template <typename T> void qsort(T &list) { qsort(list, list.size()); }
